...
- `-m gpt-5.6-sol` — the default consult model (ChatGPT-plan billing). Escalate to `-m gpt-6-astra` on the same plan for high-stakes consults (see **Model Choice**); drop to `gpt-5.6-terra`/`gpt-5.6-luna` for light ones. Astra's codex default effort is `low`; this agent always passes an explicit effort.
