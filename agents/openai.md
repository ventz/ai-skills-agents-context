...
- `-m gpt-5.6-sol` — the default consult model (ChatGPT-plan billing). Escalate to `-m gpt-6-astra` for high-stakes consults or after a plan usage-limit error (see **Model Choice**); drop to `gpt-5.6-terra`/`gpt-5.6-luna` for light ones. Astra's codex default effort is `low`; this agent always passes an explicit effort.
...
`gpt-6-astra` (GPT-6) is OpenAI's frontier model (released 2026-09-03; knowledge cutoff 2026-04-30). **Consult default (Ventz, 2026-09-12): path A — `gpt-5.6-sol` on the ChatGPT plan.** Escalate to **`gpt-6-astra` via path C** (per-run `CODEX_API_KEY`, pay-per-token) for high-stakes consults or once the plan usage limit hits — that escalation is pre-approved; name the path used in the handoff. Path B (Astra on the same plan) also works at no per-token cost but drains the shared plan allowance fastest — use it only when Ventz asks. Verified on this machine 2026-09-11:
...
| B. Astra on the ChatGPT plan | `codex exec -m gpt-6-astra …` (codex ≥ 0.153.1) | Plan allowance (Astra draws it down fastest) | ✅ works — verified 2026-09-11 on codex 0.154.0 with an edu ChatGPT plan, no API key in the environment (4,080 tokens for a one-line probe) — available, but not the routine default: it drains the shared plan allowance fastest |
| C. Astra, per-run API key | `CODEX_API_KEY="$OPENAI_API_KEY" codex exec -m gpt-6-astra …` | Pay-per-token on the API key; `~/.codex/auth.json` untouched and `codex login status` still says ChatGPT | ✅ works — **escalation path** for high-stakes consults and after a plan usage-limit error |
...
- **Never** persist API-key auth (`printenv OPENAI_API_KEY | codex login --with-api-key` — the old `--api-key` flag is gone): it moves every consult to pay-per-token. Per-run `CODEX_API_KEY` is the only API-key path; it is pre-approved for the escalation cases above and for nothing else.
