...
- `-m gpt-6-sol` — the default consult model for most work (Ventz, 2026-09-23). `-m gpt-6-luna` for light ones. `-m gpt-6-astra` **only when asked** — security audits, or Ventz names Astra (see **Model Choice**). Always try the ChatGPT plan first; **the plan doesn't offer `gpt-6-sol` / `gpt-6-luna` yet** (400 `not supported when using Codex with a ChatGPT account`, re-checked 2026-09-23 on codex 0.156.1), so on that error re-run once on the per-run API key (path B). If the key is missing or the API path fails, fall back to `-m gpt-5.6-sol` on the plan (path C). Say which path ran in the run metadata.
...
- If the plan allowance is exhausted mid-audit, follow **Plan Usage Limits** — an audit is exactly the case where path E (per-run `CODEX_API_KEY`) is justified if it can't wait; name the path used.
...
**Consult default (Ventz, 2026-09-23): `gpt-6-sol` for most projects and consults.** Rule for every model: **the ChatGPT plan first; if the plan doesn't offer the model, a per-run `CODEX_API_KEY` as the fallback** (pre-approved). For Sol that's path A → B. Only if the key is missing or the API path fails does a consult drop to `gpt-5.6-sol` on the plan (path C). **`gpt-6-astra` (GPT-6 Astra, frontier; released 2026-09-03, cutoff 2026-04-30) is the "when asked for" security auditor** (path D → E): run it for security audits Ventz requests (standing rule, no per-run approval; see **Security Audits**) or when Ventz explicitly names Astra. It is never an automatic escalation for "high-stakes" consults; bump Sol's effort to `xhigh`/`max` instead. Name the path used in the handoff.

| Path | Invocation | Billing | Status |
|---|---|---|---|
| A. GPT-6 Sol on the ChatGPT plan | `codex exec -m gpt-6-sol …` | Plan allowance | ⏳ **default, not offered yet**: 400 `not supported when using Codex with a ChatGPT account` on codex 0.154.0, 0.156.0 and 0.156.1 (2026-09-22/23, an edu ChatGPT plan). The plan's `models_cache.json` lists only Astra + the 5.6 family. Try it first on every consult; on that 400, use path B. Once it passes, set `model = "gpt-6-sol"` in `~/.codex/config.toml` |
| B. GPT-6 Sol, per-run API key | `CODEX_API_KEY="$OPENAI_API_KEY" codex exec -m gpt-6-sol -c service_tier="default" …` | Pay-per-token ($2/$10); `~/.codex/auth.json` untouched and `codex login status` still says ChatGPT | ✅ **fallback when the plan doesn't offer Sol** (today, every Sol consult). Verified 2026-09-23 (~14.9K tokens for a one-line probe; codex's system prompt dominates short runs). Same for `gpt-6-luna` |
| C. GPT-5.6 Sol on the ChatGPT plan | `codex exec -m gpt-5.6-sol …` | Plan allowance (5-hour + weekly windows) | ✅ **last resort**, only when path B can't run (no key, API auth/quota error). It was the consult default until 2026-09-23. `gpt-5.6-luna`/`terra` for light consults |
| D. Astra on the ChatGPT plan | `codex exec -m gpt-6-astra …` (codex ≥ 0.153.1) | Plan allowance (Astra draws it down fastest) | ✅ works. Verified 2026-09-11 and re-checked 2026-09-12 on codex 0.154.0, no API key in the environment (~4,060 tokens for a one-line probe). **Only when asked**: requested security audits, or Ventz names Astra |
| E. Astra, per-run API key | `CODEX_API_KEY="$OPENAI_API_KEY" codex exec -m gpt-6-astra -c service_tier="default" …` | Pay-per-token ($10/$50) | ✅ works. Fallback for a requested Astra run when the plan doesn't offer it (allowance exhausted, or Astra unavailable) |
| F. Astra via Amazon Bedrock | `model_provider = "amazon-bedrock"`, model `openai.gpt-6-astra` (us-west-2 Mantle) | AWS bill | ❓ untested; OpenAI's Codex-on-Bedrock page doesn't list Astra or web search yet |

- **Never** persist API-key auth (`printenv OPENAI_API_KEY | codex login --with-api-key` — the old `--api-key` flag is gone): it moves every consult to pay-per-token, including the ones the plan does offer. The only API-key path is a per-run `CODEX_API_KEY`, and it is pre-approved solely as the fallback when the plan doesn't offer the model (paths B, E).
- **Every API-key run MUST notify Ventz (Ventz, 2026-09-23).** Pre-approval is not silence. Put this line, in capitals, at the very top of the handoff, with the model filled in: `%%% WARNING %%% <MODEL> RAN ON THE OPENAI API KEY (PAY-PER-TOKEN) BECAUSE THE CHATGPT PLAN DOES NOT OFFER IT %%% WARNING %%%`. The parent Claude session relays it **verbatim** to Ventz in its reply, the same turn, and never softens, lowercases or drops it. It applies whenever `CODEX_API_KEY` was set, including a retry that failed.
- On API-key runs, note the models cache lists `default_service_tier: "priority"`; always pass `-c service_tier="default"` (paths B and E above do) unless priority processing was approved (medium confidence on the billing effect — check the pricing page).
...
| `gpt-6-sol` | 2 / 0.20 / 10 (cache write 2.50) | API key until the plan offers it (consult default) | none…max, default `medium`; Apr 20 2026 cutoff; Chat Completions function calling only at `none` |
...
- Plan paths (A, C, D) are metered: rolling 5-hour window plus a weekly cap; `ultra` effort and Astra burn it fastest.
...
- Report the reset time to the parent as a `BLOCKING:`/`DECIDE:` item. The limit covers the whole plan, so Astra (path D) is blocked too. Fall back to the per-run key (**path B** for Sol, **path E** for a requested Astra run; pay-per-token, pre-approved for exactly this case), or wait for the reset if the key path isn't available. Name the path used in the handoff, and never persist API-key auth.
...
- **`%%% WARNING %%%` line first, if any run used the API key.** It goes at the very top, in capitals, exactly as specified in **Model Choice**. The parent relays it verbatim.
