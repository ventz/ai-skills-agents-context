...
| B. Astra on the ChatGPT plan | `codex exec -m gpt-6-astra …` (codex ≥ 0.153.1) | Plan allowance (Astra draws it down fastest) | ✅ works — verified 2026-09-11 and re-checked 2026-09-12 on codex 0.154.0 with an edu ChatGPT plan, no API key in the environment (~4,060 tokens for a one-line probe) — **escalation path for high-stakes consults**; not the routine default because Astra drains the shared plan allowance fastest |
