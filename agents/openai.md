...
- `-m gpt-5.6-sol` — the default consult model (ChatGPT-plan billing). Escalate to `-m gpt-6-astra` on the same plan for high-stakes consults (see **Model Choice**); drop to `gpt-5.6-terra`/`gpt-5.6-luna` for light ones. **`gpt-6-sol` / `gpt-6-luna` are not on the ChatGPT plan yet** (400 `not supported when using Codex with a ChatGPT account`, codex 0.156.0, 2026-09-22) — see **Model Choice**. Astra's codex default effort is `low`; this agent always passes an explicit effort.
...
**When Ventz asks for a security audit / security review / "what could an attacker do with this", run it on `gpt-6-astra` (GPT-6), not on Sol.** This is a standing escalation — it does not need per-run approval (path B, ChatGPT plan; see **Model Choice**). Astra's frontier reasoning is worth the plan allowance here: security findings are correctness-critical and a missed vulnerability costs far more than the tokens. Audits are **on request only** — don't silently turn an ordinary consult into one, and don't route routine reviews to Astra.
...
- Sending `temperature`, `top_p`, or `logprobs` to reasoning calls — Astra always rejects them, and GPT-5.6/GPT-6 reject any `temperature` but 1 even on Chat Completions (`Unsupported value: 'temperature' does not support 0.7`, verified 2026-09-22); steer style with instructions and `text.verbosity`.
...
`gpt-6-astra` (GPT-6) is OpenAI's frontier model (released 2026-09-03; knowledge cutoff 2026-04-30). **Consult default (Ventz, 2026-09-12): path A — `gpt-5.6-sol` on the ChatGPT plan.** Escalate to **`gpt-6-astra` on the same plan (path B)** for high-stakes consults — and **always for security audits when Ventz asks for one** (standing escalation, no per-run approval needed; see **Security Audits**) — re-verified working 2026-09-12 with no API key, so escalation costs nothing per token, only plan allowance (which Astra drains fastest). **Path C (per-run `CODEX_API_KEY`, pay-per-token) is the last resort**, for when the plan allowance itself is exhausted and the consult can't wait. Both escalations are pre-approved; name the path used in the handoff. Verified on this machine 2026-09-11:

| Path | Invocation | Billing | Status |
|---|---|---|---|
| A. Sol on the ChatGPT plan | `codex exec -m gpt-5.6-sol …` | Plan allowance (5-hour + weekly windows) | ✅ **default**; hits usage limits on heavy consults |
| B. Astra on the ChatGPT plan | `codex exec -m gpt-6-astra …` (codex ≥ 0.153.1) | Plan allowance (Astra draws it down fastest) | ✅ works — verified 2026-09-11 and re-checked 2026-09-12 on codex 0.154.0 with an edu ChatGPT plan, no API key in the environment (~4,060 tokens for a one-line probe) — **escalation path for high-stakes consults and the default for requested security audits**; not the routine default because Astra drains the shared plan allowance fastest |
| C. Astra, per-run API key | `CODEX_API_KEY="$OPENAI_API_KEY" codex exec -m gpt-6-astra …` | Pay-per-token on the API key; `~/.codex/auth.json` untouched and `codex login status` still says ChatGPT | ✅ works — **last resort**: only when the plan allowance is exhausted and the consult can't wait |
...
| E. GPT-6 Sol/Luna on the ChatGPT plan | `codex exec -m gpt-6-sol …` | — | ❌ **not available** — 400 `not supported when using Codex with a ChatGPT account` on codex 0.154.0 and 0.156.0 (2026-09-22); the plan's `models_cache.json` lists only Astra + the 5.6 family. Works with a per-run `CODEX_API_KEY` (~15K tokens for a one-line probe). Re-test with `env -u OPENAI_API_KEY codex exec -m gpt-6-sol --skip-git-repo-check --ephemeral -c model_reasoning_effort=low "Reply with exactly: ok" < /dev/null`; once it passes, raise switching the consult default from `gpt-5.6-sol` to `gpt-6-sol` as a `DECIDE:` item |
...
| `gpt-6-sol` | 2 / 0.20 / 10 (cache write 2.50) | — (API key only) | none…max, default `medium`; Apr 20 2026 cutoff; Chat Completions function calling only at `none` |
...
- Report the reset time to the parent as a `BLOCKING:`/`DECIDE:` item. The limit covers the whole plan, so Astra (path B) is blocked too: either drop to a cheaper tier (`gpt-5.6-terra`/`luna`) if any allowance remains, wait for the reset, or use **path C** (Astra via per-run `CODEX_API_KEY`, pay-per-token — pre-approved for exactly this case). Name the path used in the handoff, and never persist API-key auth.
